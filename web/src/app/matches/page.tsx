...
async function getMatches() {
  try {
    return await query<any[]>(`
      ...
      GROUP BY p.id_partida, p.fecha_partida, mapa, p.resultado_puntuacion, p.duracion_minutos
...
export default async function MatchesPage() {
  ...
  return (
    <div>
      <h1 style={{ fontFamily:'Rajdhani,sans-serif', fontSize:28, marginBottom:4 }}>
        CS2 Matches
      </h1>
        ...
        <table style={{ width:'100%', fontSize:13, borderCollapse:'collapse' }}>
          <thead>
            <tr style={{ borderBottom:'1px solid var(--bg-border)' }}>
              {['Mode','Date','Map','Score','Players','K','D','A','Duration'].map(h => (
          ...
          <tbody>
            {matches.length === 0 ? (
              ...
              <tr key={m.id_partida}
                ...
                <td style={{ padding:'9px 12px', fontWeight:500, color:'var(--t1)' }}>
                  {m.mapa ?? '—'}
                </td>
                <td style={{ padding:'9px 12px', fontFamily:'IBM Plex Mono,monospace',
                              fontSize:12, color:'var(--t1)', fontWeight:600 }}>
                  {m.resultado_puntuacion ?? '—'}
                </td>
                <td style={{ padding:'9px 12px', color:'var(--t3)', fontSize:12 }}>
                  {m.total_jugadores ?? 0}
                </td>
                <td style={{ padding:'9px 12px', fontFamily:'IBM Plex Mono,monospace',
